...
*   **수능형 문제** (국어, 사회 영역 윤리, 정치, 사회와 비슷한 문제)
...
*   **대응:** 원본 데이터 분포를 따르도록 전략을 수정하여 훈련 셋 성능은 방어했으나, 테스트 셋에서의 성능 하락은 해결되지 않음 (t-SNE, UMAP 시각화 결과 훈련 셋과 테스트 셋의 질문 구성이 임베딩 스페이스 내에서 서로 다른 영역에 분포하는 문제 확인).
